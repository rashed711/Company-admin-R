...
const CustomerStatementDetail = () => {
    ...
    const handleShare = async () => {
        ...
        if (!pdfBlob) return;
        const pdfFile = new File([pdfBlob], `statement-${customer.id}.pdf`, { type: 'application/pdf' });
        
        if (navigator.share && navigator.canShare && navigator.canShare({ files: [pdfFile] })) {
            await navigator.share({ files: [pdfFile], title: t('account_statement') });
        } else {
    ...
    if (!customer) {
        return (
            <div className="text-center p-8"><h1 className="text-2xl font-bold mb-4">Customer Not Found</h1><Button as={Link} to="/contacts/customers" variant="primary">{t('back_to_list')}</Button></div>
        );
    }
