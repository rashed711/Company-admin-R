...
const ReceiptVoucherDetail = () => {
    ...
    const voucher = mockReceiptVouchersData.find(v => v.id === parseInt(id || '0'));
    const customer = mockCustomersData.find(c => c.id === voucher?.customer_id);

    const generatePDF = (voucherData: ReceiptVoucher) => {
        const doc = new jsPDF();
        ...
        // Header
        doc.setFontSize(22);
        doc.text(processText(t('receipt_voucher')), 105, 20, { align: 'center' });
        doc.setFontSize(10);
        ...
        let startY = 45;
        doc.setFontSize(12);
        doc.text(processText(`${t('receipt_voucher')} #: ${voucherData.id}`), xPos, startY, { align });
        ...
        doc.text(processText(`${t('customer')}: ${voucherData.customer_name}`), xPos, startY, { align });
        startY += 7;
        if(customer?.phone) doc.text(processText(`${t('phone')}: ${customer.phone}`), xPos, startY, { align });
        startY += 10;
        ...
        doc.save(`receipt-voucher-${voucherData.id}.pdf`);
    };
    ...
    if (!voucher) {
        return (
            <div className="text-center p-8">
                <h1 className="text-2xl font-bold mb-4">Receipt Voucher Not Found</h1>
                <Button as={Link} to="/accounting/receipt-vouchers" variant="primary">{t('back_to_list')}</Button>
            </div>
        );
    }
    ...
    return (
        <div>
            <div className="flex flex-wrap justify-between items-center mb-6 no-print gap-2">
                <h1 className="text-3xl font-bold">تفاصيل سند القبض</h1>
                <div className="flex flex-wrap gap-2">
                    <Button as={Link} to={`/accounting/receipt-vouchers/${voucher.id}/edit`} variant="outline">{t('edit')}</Button>
                    ...
                    <Button as={Link} to="/accounting/receipt-vouchers" variant="outline">{t('back_to_list')}</Button>
                </div>
            </div>
                ...
                <div id="print-area" className="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-8 md:p-12 max-w-4xl mx-auto text-gray-900 dark:text-gray-200">
                     <header className="flex justify-between items-start mb-8 border-b dark:border-gray-700 pb-4">
                        ...
                        <div className={`text-${isRTL ? 'left' : 'right'}`}>
                            <h2 className="text-3xl font-bold uppercase">{t('receipt_voucher')}</h2>
                        </div>
                    </header>
                    ...
                    <section className="flex justify-between items-center mb-8 text-sm">
                        ...
                        <div>
                            <p><span className="font-semibold">{t('receipt_voucher')} #:</span> {voucher.id}</p>
