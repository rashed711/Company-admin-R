...
const SupplierInvoiceEdit = () => {
    ...
    const handleItemChange = (index: number, e: ChangeEvent<HTMLInputElement>) => {
        ...
        if(name === 'description' || name === 'unit' || name === 'product_name') {
            itemToUpdate[name as 'description' | 'unit' | 'product_name'] = value;
        } else if (name === 'quantity' || name === 'price') {
            itemToUpdate[name as 'quantity' | 'price'] = parseFloat(value) || 0;
        }
    ...
    const handleSave = () => {
        if (invoice) {
            ...
            console.log("Saving supplier invoice:", finalInvoice);
            alert('Changes saved (check console log).');
            const targetId = isNew ? invoice.id : id;
            navigate(`/invoices/purchases/${targetId}`);
        }
    };
