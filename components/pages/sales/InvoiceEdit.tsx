...
import { Invoice, InvoiceItem, LocaleKey, Product, Customer } from '../../../types';
...
import { getInvoiceById, addInvoice, updateInvoice, getProducts, getCustomers } from '../../../services/api';
...
const InvoiceEdit = () => {
    ...
    const handleSave = async () => {
        ...
        const { error } = isNew
            ...
            : await updateInvoice(parseInt(id!), finalInvoice);
                    ...
                    <div>
                        <label htmlFor="customer_id" className="block mb-2 text-sm font-medium text-[rgb(var(--color-text-primary))]">{t('customer')}</label>
                         <SearchableSelect
                            ...
                            value={invoice.customer_id?.toString() || ''}
